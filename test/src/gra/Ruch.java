...
public class Ruch {
	...
	public static boolean mogeRuszycPrawo(String kto, Pole[][] plansza, int pionekWiersz, int pionekKolumna, boolean czyGracz) {
		...
		if(pionekKolumna == Stale.ROZMIAR_PLANSZY) {
			...
			if(czyNaPlanszy((pionekWiersz+skokWiersz), (pionekKolumna+skokKolumna))
			   ...
			   moge = true;
				...
				if(czyGracz && !Stale.POLE_PUSTE.equals(plansza[pionekWiersz][pionekKolumna].dajPionek())) 
					System.out.println("Prawe pole pionka "+
										plansza[pionekWiersz][pionekKolumna].dajPionek()+
										", jest zajête.");
				moge = false;
			}
